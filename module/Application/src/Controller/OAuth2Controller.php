...
final class OAuth2Controller extends AbstractActionController
{
    ...
    public function __construct(UserService $userService, array $config, AuthenticationService $authenticationService)
    {
    ...
    public function callbackAction(): Response
    {
        ...
        if (null !== $authCode) {
            try {
                ...
                //Do a manual Guzzle Request
                $guzzle  = new Client();
                $request = $guzzle->request(
                    'GET',
                    $session->profileUrl . '/1234',
                    [
                ...
                //Try to see if we can find the user
                $user = $this->userService->findOrCreateUserFromGenericUser($genericUser);

                //we have the user now, so we can create a bearer token
                //Create bearer token
